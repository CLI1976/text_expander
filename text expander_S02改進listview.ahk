...
AddNewSnippet(*)
{
    ...
    SaveNewHandler(*)  ; 改名為 SaveNewHandler
    {
        keyword := keywordEdit.Value
        phrase := phraseEdit.Value
        if (keyword != "" && phrase != "") {
            textSnippets[keyword] := phrase
            LV.Add(, keyword, phrase)
            TV.Add(keyword, selectedNode)
...
SaveSnippets()
{
    ...
    try {
        ...
        ; 遍歷 TreeView 的所有節點
        currentNode := TV.GetNext()  ; 獲取第一個節點
        while currentNode {
            ; 獲取節點文字
            ...
            while childNode {
                key := TV.GetText(childNode)
                if textSnippets.Has(key) {
                    encodedValue := StrReplace(textSnippets[key], "`n", "<<NEWLINE>>")
                    f.Write(key "=" encodedValue "`n")
                }
                childNode := TV.GetNext(childNode)
            }
            f.Write("`n")  ; 分組之間加入空行
