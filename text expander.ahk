...
EditSnippet(*) {
    ...
    ; 編輯日誌
; [已移除 log]     ManageLogFile(A_ScriptDir "\edit_log.txt", "=== 開始編輯 ===")
; [已移除 log]     ManageLogFile(A_ScriptDir "\edit_log.txt", "原始值：")
; [已移除 log]     ManageLogFile(A_ScriptDir "\edit_log.txt", "群組路徑: " oldGroupPath)
; [已移除 log]     ManageLogFile(A_ScriptDir "\edit_log.txt", "顯示關鍵字: " displayKeyword)
; [已移除 log]     ManageLogFile(A_ScriptDir "\edit_log.txt", "實際關鍵字: " oldKeyword)
; [已移除 log]     ManageLogFile(A_ScriptDir "\edit_log.txt", "短語: " oldPhrase)
; [已移除 log]     ManageLogFile(A_ScriptDir "\edit_log.txt", "是否為memo: " isMemo)
...
; [已移除 log]     if FileExist(A_ScriptDir "\edit_log.txt")
; [已移除 log]         FileDelete(A_ScriptDir "\edit_log.txt")

    SaveEditHandler(*) {
        keyword := keywordEdit.Value
        phrase := phraseEdit.Value
        
; [已移除 log]         FileAppend("=== 儲存編輯 ===`n", A_ScriptDir "\edit_log.txt")
; [已移除 log]         FileAppend("新值：`n", A_ScriptDir "\edit_log.txt")
; [已移除 log]         FileAppend("關鍵字: " keyword "`n", A_ScriptDir "\edit_log.txt")
...
; [已移除 log]                 FileAppend("舊關鍵字已從 Map 中刪除`n", A_ScriptDir "\edit_log.txt")
            ...
            while currentNode && !found {
                if (TV.GetText(currentNode) = oldKeyword) {
                    ...
                    found := true
; [已移除 log]                     FileAppend("在頂層找到並更新節點`n", A_ScriptDir "\edit_log.txt")
                } else {
...
; [已移除 log]                 FileAppend("熱字串已更新`n", A_ScriptDir "\edit_log.txt")
            ...
            ; 5. 保存所有更改
            SaveSnippets()
; [已移除 log]             FileAppend("所有更改已保存`n", A_ScriptDir "\edit_log.txt")
...
; [已移除 log]             FileAppend("在子層找到並更新節點: " oldText " -> " newText "`n", A_ScriptDir "\edit_log.txt")
            return true
        }
...
ShowSearchWindow(*) {
    ...
    resultList.OnEvent("ItemSelect", (*) => UpdatePreviewFromSearch(resultList))

    searchGui.Show()
}

; 從搜尋結果更新主視窗的 Preview
UpdatePreviewFromSearch(resultList) {
    row := resultList.GetNext()
    if row {
        phrase := resultList.GetText(row, 3)  ; 第3欄是 Phrase
        previewEdit.Value := phrase
    }
}
...
JumpToItem(resultList) {
    ...
    Loop LV.GetCount() {
        if (LV.GetText(A_Index, 2) = displayKey) {
...
LoadSnippetsAsync() {
; [已移除 log]     ManageLogFile(A_ScriptDir "\load_debug.txt", "=== 開始異步載入 ===")
...
; [已移除 log]     ManageLogFile(A_ScriptDir "\load_debug.txt", "=== 載入完成 ===")
    progressGui.Destroy()
}
